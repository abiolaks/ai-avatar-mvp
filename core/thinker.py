...
"""
    ...
    def _update_collected_info(self, user_text):
        ...
        elif "data scientist" in text_lower or "data science" in text_lower or "detached scientist" in text_lower or "data size" in text_lower: # Handle mishearing
        ...
        # Level detection
        if any(w in text_lower for w in ["beginner", "new", "scratch", "starting", "begin", "novice", "no experience"]):
        ...
        skills = []
        common_skills = ["python", "html", "css", "javascript", "react", "java", "tensorflow", "sql", "git"]
        for skill in common_skills:
            # Simple negation check
            if f"no {skill}" in text_lower or f"not {skill}" in text_lower:
                continue
            if skill in text_lower:
                skills.append(skill.upper() if skill in ["html", "css", "sql", "git"] else skill.capitalize())
        ...
        while attempt < max_retries:
            ...
            if not is_json_action:
                ...
                if "ask the user" in content.lower():
                    ...
                    # Stronger correction
                    self.history.append({"role": "system", "content": "SYSTEM: You failed the rule. Do NOT describe the question. ASK it directly. Example: 'What are your skills?'"})
                    continue
                
                # INTERNAL STATE LEAKAGE FILTER
                if "action:" in content.lower() or "params:" in content.lower():
                    logger.warning("Detected internal state leakage (Action/Params). Retrying...")
                    self.history.append({"role": "assistant", "content": content})
                    self.history.append({"role": "system", "content": "SYSTEM: You are outputting internal debug text. STOP. Just ask the question in natural English."})
                    continue
                ...
                if any(phrase in content for phrase in [
                    ...
                    if not content or len(content) < 10:
                        ...
                        self.history.append({"role": "assistant", "content": content})
                        self.history.append({"role": "system", "content": "SYSTEM: Do NOT output debug info about state. Just ask the question naturally."})
                        continue
            ...
            if action == "recommend":
                ...
                if not force_recommendation and len(self.history) < 10:  # Increased from 6 to 10
                ...
                collected_count = sum(1 for v in self.collected_info.values() if v is not None)
                
                if collected_count < 4:
                    # Model is hallucinating! Reject hard.
                    missing_fields = [k for k, v in self.collected_info.items() if v is None]
                    logger.warning(f"HALLUCINATION DETECTED. User only provided {collected_count}/4 fields. Missing: {missing_fields}")
                ...
                for _ in range(max_final_retries):
                    ...
                    if "{" in final_text and "}" in final_text and "action" in final_text:
                        logger.warning("Final response was still JSON. Retrying...")
                        self.history.append({"role": "assistant", "content": final_text})
                        self.history.append({"role": "system", "content": "SYSTEM: STOP. Do NOT output JSON. Just write a friendly message to the user."})
                        continue
                    else:
                        break
                ...
                retry_msg = "Do NOT output JSON. Ask the user a question in natural English."
                self.history.append({"role": "assistant", "content": content})
                self.history.append({"role": "system", "content": retry_msg})
                continue # Loop again
